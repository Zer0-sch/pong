...
def spawn_ball(direction):
    ...
    if direction == LEFT:
        direction_sign = -1
    ball_vel = direction_sign * random.randrange(2, 6), -random.randrange(2, 5)
...
def draw(canvas):
    ...
    # draw ball
    canvas.draw_circle(ball_pos, BALL_RADIUS, 10, 'grey', 'white')
    ...
    canvas.draw_text(str(score2), (750, 50), 50, 'blue')
    
    # draw timer
    timer += 1
    if timer > 0 and timer <= 5:
        canvas.draw_text(str(timer), (500, 350), 50, 'blue')
        timer = 0
